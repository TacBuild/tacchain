...
RESP=$(tacchaind tx wasm submit-proposal store-instantiate "$DIR/../../tests/testdata/reflect_2_0.wasm" \
  '{}' --label="testing" \
  --title "testing" --summary "Testing" --deposit "1000000000utac" \
  --admin $(tacchaind keys show -a validator --keyring-backend=test) \
  --amount 123utac \
  --keyring-backend=test \
  --from validator --gas auto --gas-adjustment=1.5 -y   --node=http://localhost:26657 -b sync -o json)
echo $RESP
sleep 6
