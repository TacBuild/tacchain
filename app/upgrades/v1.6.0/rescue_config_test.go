...
// init configures the global SDK bech32 prefix to "tac" so that test
// addresses can be parsed by sdk.AccAddressFromBech32 inside parseRescueEntries.
func init() {
...
func TestParseRescueEntries_Errors(t *testing.T) {
	cases := []struct {
		...
		{
			name:    "missing vesting_migration",
			info:    `{"binaries":{}}`,
			wantErr: "missing 'vesting_migration'",
		},
		{
			name:    "empty rescues",
			info:    `{"vesting_migration":{"rescues":[]}}`,
			wantErr: "is empty",
		},
		{
			name:    "invalid old bech32",
			info:    `{"vesting_migration":{"rescues":[{"old":"not-bech32","new":"tac1gepr027cw2l606z8grrsagzznw9esfyvz7mrxu"}]}}`,
			wantErr: "rescues[0].old",
		},
		{
			name:    "invalid new bech32",
			info:    `{"vesting_migration":{"rescues":[{"old":"tac12t0efd0ylr4mlz4n0rm367qpt09g6yxq0pqnkt","new":"x"}]}}`,
			wantErr: "rescues[0].new",
		},
		{
			name:    "old==new",
			info:    `{"vesting_migration":{"rescues":[{"old":"tac12t0efd0ylr4mlz4n0rm367qpt09g6yxq0pqnkt","new":"tac12t0efd0ylr4mlz4n0rm367qpt09g6yxq0pqnkt"}]}}`,
